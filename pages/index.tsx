...
export default function Home({ token, data }) {
  const green = data.features.filter(location => location.properties.status == 'Good').length
  const yellow = data.features.filter(location => location.properties.status == 'Medium').length
  const red = data.features.filter(location => location.properties.status == 'Bad').length
